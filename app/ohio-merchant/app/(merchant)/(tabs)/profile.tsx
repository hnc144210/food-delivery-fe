    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.toggleCard}>
        <View style={styles.toggleLeft}>
          <View style={[styles.dot, { backgroundColor: isOpen ? '#22c55e' : '#ccc' }]} />
          <View>
            <Text style={styles.toggleLabel}>Trạng thái cửa hàng</Text>
            <Text style={styles.toggleSub}>{isOpen ? 'Đang mở cửa' : 'Đã đóng cửa'}</Text>
          </View>
        </View>
        <Switch
          value={isOpen}
      ...
      <View style={styles.menuGroup}>
        <MenuItem icon="create-outline"     label="Thông tin cửa hàng" onPress={() => router.push('/(merchant)/store-info')} />
        <MenuItem icon="time-outline"       label="Giờ mở cửa"         value="07:00 – 22:00" onPress={() => router.push('/(merchant)/opening-hours')} />
        <MenuItem icon="grid-outline"       label="Sắp xếp danh mục"   onPress={() => router.push('/(merchant)/category-layout')} />
      </View>
      ...
      <View style={styles.menuGroup}>
        ...
        <MenuItem icon="document-text-outline" label="Điều khoản & Chính sách" onPress={() => {}} />
        <MenuItem icon="information-circle-outline" label="Phiên bản" value="1.0.0" />
      </View>

      <View style={styles.menuGroup}>
        <MenuItem icon="log-out-outline" label="Đăng xuất" danger onPress={() => { clearUser(); router.replace('/(auth)/login'); }} />
      </View>
      </ScrollView>
      </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  storeCard:       { backgroundColor: '#fff', borderRadius: 16, padding: 16, flexDirection: 'row', gap: 14, alignItems: 'center' },
  avatar:          { width: 64, height: 64, borderRadius: 16, backgroundColor: CREAM, alignItems: 'center', justifyContent: 'center' },
  ...
  toggleCard:      { backgroundColor: '#fff', borderRadius: 14, padding: 16, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  toggleLeft:      { flexDirection: 'row', alignItems: 'center', gap: 10 },
  dot:             { width: 10, height: 10, borderRadius: 5 },
  toggleLabel:     { fontSize: 15, fontWeight: '600', color: '#1a1a1a' },
  toggleSub:       { fontSize: 12, color: '#888', marginTop: 2 },
  sectionLabel:    { fontSize: 12, fontWeight: '700', color: '#999', letterSpacing: 0.5, marginLeft: 4 },
  menuGroup:       { backgroundColor: '#fff', borderRadius: 14, overflow: 'hidden' },
  menuItem:        { flexDirection: 'row', alignItems: 'center', gap: 12, padding: 14, borderBottomWidth: 1, borderBottomColor: '#f5f5f5' },
  menuIcon:        { width: 34, height: 34, borderRadius: 10, backgroundColor: CREAM, alignItems: 'center', justifyContent: 'center' },
