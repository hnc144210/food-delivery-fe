//components/features/ProductOptions.tsx
...
export function ProductOptions({
  ...
  const getOpacity = (index: number) => {
    ...
    if (
      ...
      option.max_selection != null &&
      selectedCount === option.max_selection
    ) {
      return 0.6;
    }
    return 1;
  ...
  const getDisable = (index: number) => {
    return (
      !selectedOption[index] &&
      option.max_selection != null &&
      selectedOption.filter(Boolean).length === option.max_selection
    );
  };
  return (
    <View style={styles.container}>
      ...
      {option.options?.map((value, index) => (
        <TouchableOpacity
          key={index}
          style={[styles.optionRow, { opacity: getOpacity(index) }]}
          onPress={() => {
