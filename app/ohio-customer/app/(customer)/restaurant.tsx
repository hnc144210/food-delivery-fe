import { Image, StyleSheet, TouchableOpacity, View, Text, ScrollView, FlatList, ActivityIndicator } from "react-native";
...
export default function Restaurant() {
    ...
    const restaurantProducts = products?.items.filter((product) => product.merchantId === id);

    if (isLoading || isProductsLoading || isReviewsLoading) {
        return (
            <View style={[styles.container, { justifyContent: 'center', alignItems: 'center' }]}>
                <ActivityIndicator size="large" color="#EE4D2D" />
                <Text style={{ marginTop: 12, color: '#888' }}>Đang tải thông tin nhà hàng...</Text>
            </View>
        );
    }
    ...
    return (
        <ScrollView style={styles.container}>
            <View style={{ width: '100%', height: 320, position: 'relative' }}>
                <Image source={{ uri: restaurantData.storeBannerUrl }} style={{ width: '100%', height: 320, backgroundColor: 'lightgray' }} />
                <View style={{ position: 'absolute', top: 50, left: 20, }}>
                    <ReturnButton onpressfunction={router.back} />
                </View>
                <View style={styles.restaurantinfo_container}>
                    <Image source={{ uri: restaurantData.storeLogoUrl }} style={{ width: 50, height: 50, borderRadius: 10, marginRight: 10 }} />
                    <View>
                        <Text style={styles.restaurantname}>{restaurantData.storeName}</Text>
                        ...
                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 3 }}>
                            <AntDesign name="disconnect" size={15} color={restaurantData.isOpen ? 'green' : 'red'} />
            ...
            {isInfo && (
                <View style={styles.body}>
                    ...
                    <View style={{ flexDirection: 'column', gap: 20 }}>
                        <Text>{restaurantData.openingTime} - {restaurantData.closingTime}</Text>
                    </View>
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginVertical: 10 }}>
                        <Text style={{ fontWeight: 'bold', fontSize: 17 }}>Chứng nhận hoạt động</Text>
                        <View style={{ height: 1, flex: 1, backgroundColor: 'lightgray', marginLeft: 10 }} />
                    </View>
                    <View style={{ flexDirection: 'column', gap: 20 }}>
                        <Text>{restaurantData.status == 'Pending' ? 'Đang chờ xử lý' : restaurantData.status == 'Approved' ? 'Đã duyệt' : restaurantData.status == 'Rejected' ? 'Đã từ chối' : 'Nghi ngờ'}</Text>
                    </View>
                </View>
            )}
