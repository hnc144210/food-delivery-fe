...
export default function Review() {
  ...
  const updateReview = (
    productId: string,
    stateUpdate: Partial<ReviewState>,
  ) => {
  ...
  const submitReviewsMutation = useMutation({
    mutationFn: async () => {
      ...
      const promises = orderDetail.items.map((item) => {
        const itemReview = reviewsState[item.productId] || {
          rating: 5,
          comment: "",
          image: null,
        };
    ...
    >
      <View style={styles.container}>
        ...
        >
          ...
          <Text style={styles.mainPromptText}>
            Bạn hài lòng với các món ăn chứ?
          </Text>
          ...
          {orderDetail.items.map((item) => (
            <ReviewCard
              ...
              onChange={(stateUpdate) =>
                updateReview(item.productId, stateUpdate)
              }
            />
          ))}
