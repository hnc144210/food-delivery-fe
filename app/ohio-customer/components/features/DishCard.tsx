...
export default function DishCard({ dish, onToggleAvailable, onEdit, onManageOptions }: DishCardProps) {
    ...
    <View style={styles.card}>
      ...
      <View style={styles.topRow}>
        ...
        <View style={styles.info}>
          ...
          <Text style={styles.price}>{dish.price.toLocaleString('vi-VN')}đ</Text>
          <Text style={styles.description} numberOfLines={2}>{dish.description}</Text>
          <Text style={[styles.availableLabel, { color: dish.isAvailable ? '#10B981' : '#AAA' }]}>
        ...
        <>
          ...
          <View style={styles.optionsSection}>
            <View style={styles.optionsHeader}>
              <TouchableOpacity style={styles.manageBtn} onPress={() => onManageOptions(dish.id)}>
              ...
              <View style={styles.toppingsList}>
                {dish.toppings.map((topping) => (
                  <View key={topping.id} style={styles.toppingRow}>
                    ...
                    <Text style={styles.toppingPrice}>+{topping.price.toLocaleString('vi-VN')}đ</Text>
                  </View>
                ))}
              </View>
            )}
...
const styles = StyleSheet.create({
  ...
  chip: { backgroundColor: '#FEF3E8', borderRadius: 20, paddingHorizontal: 10, paddingVertical: 4 },
  chipText: { fontSize: 12, color: ORANGE, fontWeight: '600' },
  addChip: { borderWidth: 1, borderColor: '#DDD', borderRadius: 20, paddingHorizontal: 10, paddingVertical: 4, borderStyle: 'dashed' },
