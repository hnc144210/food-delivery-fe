...
function CompactOrderCard({ order, onReady }: Props) {
  ...
  const buttonConfig = (() => {
    if (order.status === 'preparing') {
      return { label: 'Ready for Pick-up', color: ORANGE, onPress: () => onReady?.(order.id) }
    }
    if (order.deliverySubStatus === 'waiting_pickup') {
      return { label: 'Waiting for Pick-up', color: '#F59E0B', onPress: undefined }
    }
...
const styles = StyleSheet.create({
  ...
  // Full card
  fullBody: { padding: 16 },
  fullTopRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  orderNumber: { fontSize: 12, color: '#AAA', fontWeight: '500' },
  tag: { backgroundColor: ORANGE, borderRadius: 6, paddingHorizontal: 8, paddingVertical: 3 },
  tagText: { fontSize: 11, color: '#fff', fontWeight: '700' },
  customerName: { fontSize: 16, fontWeight: '700', color: '#1A1A1A', marginTop: 4 },
  ...
  notesBox: { backgroundColor: CREAM, borderRadius: 8, padding: 10, marginTop: 10 },
  notesText: { fontSize: 12, color: '#666' },
  divider: { height: 1, backgroundColor: '#F2F2F2', marginVertical: 14 },
  fullFooter: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  total: { fontSize: 18, fontWeight: '800', color: '#1A1A1A' },
  paymentBadge: { backgroundColor: '#F0F0F0', borderRadius: 4, paddingHorizontal: 6, paddingVertical: 2, marginTop: 3, alignSelf: 'flex-start' },
  paymentBadgeText: { fontSize: 10, color: '#888', fontWeight: '700' },
  actions: { flexDirection: 'row', gap: 8 },
  rejectBtn: { borderWidth: 1.5, borderColor: '#E0E0E0', borderRadius: 10, paddingHorizontal: 14, paddingVertical: 9 },
  rejectLabel: { fontSize: 13, fontWeight: '600', color: '#555' },
  acceptBtn: { backgroundColor: ORANGE, borderRadius: 10, paddingHorizontal: 14, paddingVertical: 9 },
